...
def md_to_html(text):
    ...
    def flush_table():
        ...
        for row_index, cells in enumerate(table_rows):
            if all(re.fullmatch(r':?-{3,}:?', cell.replace(' ','')) for cell in cells): continue
            tag='th' if row_index == 0 else 'td'
            rows.append('<tr>'+''.join(f'<{tag}>'+html.escape(cell.strip())+f'</{tag}>' for cell in cells)+'</tr>')
        if rows: out.append('<div class="reading-table"><table><thead>'+rows[0]+'</thead><tbody>'+''.join(rows[1:])+'</tbody></table></div>')
        table_rows=[]
...
def questions_html(items):
    ...
    for i,item in enumerate(items):
        if isinstance(item, dict):
            label=item.get('label','學習焦點'); q=item.get('question','')
            options=[option.get('text','') if isinstance(option, dict) else str(option) for option in item.get('options',[])]
...
def build_article(d, md):
    def normalized_text(value, fallback=''):
        if isinstance(value, str):
            # Clean Python/JSON-like list fragments accidentally emitted by AI.
            def list_text(match):
                return '；'.join(match.group(1).split("', '") if match.group(1) else [])
            value = re.sub(r"\['([^\]]*)'\]", list_text, value)
            return value.replace('\\"', '"').replace('。；', '。').replace('；。', '。')
        if isinstance(value, dict):
            return ' '.join(normalized_text(item) for item in value.values() if item)
        if isinstance(value, list):
            return '；'.join(normalized_text(item) for item in value if item)
        return fallback if value is None else str(value)

    # AI-generated records can contain structured fields or explicit nulls,
    # while the HTML template requires string replacements.
    d = dict(d)
    d['title'] = normalized_text(d.get('title'), '未命名文章')
    d['summary'] = normalized_text(d.get('summary'))
    d['seo_title'] = normalized_text(d.get('seo_title'), d['title'] + '｜精萃領導™學習中心')
    d['meta_description'] = normalized_text(d.get('meta_description'), d['summary'])
    d['subtitle'] = normalized_text(d.get('subtitle'), '把觀點帶回一個可觀察的工作行動')
    d['start_prompt'] = normalized_text(d.get('start_prompt'), '讀完這篇文章後，你想帶回哪一個工作情境？')
    d['category'] = normalized_text(d.get('category'), '未分類')
    case_value = d.get('case')
    if isinstance(case_value, str) and case_value.lstrip().startswith('{'):
        try:
            parsed_case = json.loads(case_value)
            if isinstance(parsed_case, dict):
                case_value = parsed_case
        except json.JSONDecodeError:
            pass
    if isinstance(case_value, dict):
        d['case'] = '<br><br>'.join(normalized_text(item) for item in case_value.values() if item)
    else:
        d['case'] = normalized_text(case_value)
    d['case'] = d['case'].replace('。；', '。').replace('；。', '。')
    d['hero_image_alt'] = normalized_text(d.get('hero_image_alt'), d['title'])

    t=TEMPLATE.read_text()
    t=t.replace(
        '<footer class="site-footer"><div><strong>精萃領導™學習中心</strong><a href="https://leading4elite.com/about_wesley/" target="_blank" rel="noopener">林祖威教練</a></div>',
        '<footer class="site-footer"><div><strong>精萃領導™學習中心</strong><a href="https://leading4elite.com/about_wesley/" target="_blank" rel="noopener">林祖威教練</a><a href="https://mail.google.com/mail/?view=cm&amp;fs=1&amp;to=wesley.lin%40leading4elite.com" target="_blank" rel="noopener">wesley.lin@leading4elite.com</a><a class="subscribe-link" href="'+SUBSCRIPTION_URL+'" target="_blank" rel="noopener">訂閱學習更新</a></div>'
    )
    tools=tools_html(d.get('tools',[])); tool_parts=tools.split('</aside>')
...
def main():
    ...
    for folder in sorted(ARTICLES.iterdir()):
        ...
        embedded_audio=d.get('audio_data','')
        if not audio.is_file() and isinstance(embedded_audio, str) and embedded_audio.startswith('data:audio/mpeg;base64,'):
            audio.parent.mkdir(parents=True, exist_ok=True)
            audio.write_bytes(base64.b64decode(embedded_audio.split(',', 1)[1]))
    ...
    index=index.replace('v=20260814pwa1','v=20260917-footer-qr')
    index=index.replace('.map-group{display:none;grid-template-columns:repeat(2,1fr);gap:16px}', '.map-group{display:none;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}')
    index=index.replace('.map-card{display:block;color:var(--ink);padding:28px;', '.map-card{display:block;min-width:0;box-sizing:border-box;color:var(--ink);padding:28px;')
    index=index.replace('.map-card h3{font:clamp(21px,2.2vw,28px)/1.2 Georgia;margin:12px 0 8px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}', '.map-card h3{font:clamp(21px,2.2vw,28px)/1.2 Georgia;margin:12px 0 8px;white-space:normal;min-height:2.4em;overflow:hidden}')
    index=index.replace('.map-card{display:block;min-width:0;box-sizing:border-box;color:var(--ink);padding:28px;', '.map-card{display:flex;flex-direction:column;min-width:0;box-sizing:border-box;color:var(--ink);padding:28px;')
    index=index.replace('.map-card-link{display:block;color:var(--ink);text-decoration:none}', '.map-card-link{display:flex;flex:1;flex-direction:column;color:var(--ink);text-decoration:none}')
    index=index.replace('.map-card p{color:var(--muted);font-size:14px}', '.map-card p{color:var(--muted);font-size:14px}.map-card-link span{margin-top:auto;padding-top:18px}')
    index=index.replace('.home-footer{max-width:1400px;margin:60px auto 0;padding-left:calc(5vw + 320px);padding-right:5vw}', '.home-footer{max-width:1400px;margin:60px auto 0;padding-left:5vw;padding-right:5vw}')
    ...
    admin=admin.replace('assets/admin.js?v=20260908-unique-published','assets/admin.js?v=20260912-audio-quota-fix').replace('assets/admin.js?v=20260908-published-video-sync','assets/admin.js?v=20260912-audio-quota-fix').replace('assets/admin.js?v=20260910-cloud-source','assets/admin.js?v=20260912-audio-quota-fix').replace('assets/admin.js?v=20260910-quick-scan-quality','assets/admin.js?v=20260912-audio-quota-fix')

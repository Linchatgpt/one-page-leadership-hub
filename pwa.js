(function () {
  ...
  function addInstallPrompt() {
    ...
    bar.innerHTML = '<strong>把學習中心放到裝置上</strong><span class="pwa-install-copy">下次可直接回來繼續你的工作練習。</span><button type="button" class="pwa-install-button">安裝到桌面</button><button type="button" class="pwa-install-close" aria-label="關閉安裝提示">×</button>';
    ...
    bar.querySelector('.pwa-install-button').addEventListener('click', async function () {
      if (deferredPrompt) {
        ...
        bar.querySelector('.pwa-install-copy').textContent = 'iPhone／iPad：在 Safari 點分享，再選「加入主畫面」。';
      }
    });

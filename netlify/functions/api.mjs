...
async function chat(system, user, max_tokens = 3000, asJson = true, modelOverride = '') {
  ...
  const request = { model: modelOverride || process.env.AI_MODEL || 'gpt-5.6-luna', reasoning_effort: asJson ? 'none' : (process.env.AI_REASONING_EFFORT || 'low'), messages: [{ role: 'system', content: system }, { role: 'user', content: user }], max_completion_tokens: max_tokens };
...
export default async (event, context, forcedPath = '') => {
  try {
    ...
    const method = event.method || event.httpMethod || event.requestContext?.http?.method || event.request?.method || (path === '/health' || path === '/list-drafts' ? 'GET' : 'POST');
    ...
    if (path === '/archive-article') { const id = String(payload.id || ''); if (!/^article_\d+$/.test(id)) return json(400, { error: '封存文章必須保留原文章 ID' }); const current = await store().get(id, { type: 'json' }); if (!current) return json(404, { error: '找不到已發布文章' }); await getStore({ name: 'leadership-article-drafts', consistency: 'strong' }).setJSON(id, { ...current, ...payload, id, status: 'archived', archived_at: new Date().toISOString(), updated_at: new Date().toISOString() }); await store().delete(id); return json(200, { ok: true, id, status: 'archived' }); }
    if (path === '/restore-archived') { const id = String(payload.id || ''); if (!/^article_\d+$/.test(id)) return json(400, { error: '封存文章 ID 無效' }); const archived = await getStore({ name: 'leadership-article-drafts', consistency: 'strong' }).get(id, { type: 'json' }); if (!archived || archived.status !== 'archived') return json(404, { error: '找不到封存文章' }); await getStore({ name: 'leadership-article-drafts', consistency: 'strong' }).setJSON(id, { ...archived, id, status: 'draft', updated_at: new Date().toISOString() }); return json(200, { ok: true, id, status: 'draft' }); }

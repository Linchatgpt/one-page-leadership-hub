...
async function generateArticle(payload) {
  ...
  const result = await chat(system, JSON.stringify({ title: clean(payload.title || ''), category: clean(payload.category || ''), source_text: clean(String(payload.source_text || '').slice(0, 7000)) }), 2500);

import { normalizeQuickScan } from './learning-quality.mjs';
...
export default function renderArticle(article = {}) {
  ...
  const tools = [...new Map((Array.isArray(a.tools) ? a.tools : []).filter((tool) => tool && (tool.explanation || tool.body || (tool.steps && tool.steps.length))).map((tool) => [text(tool.title || tool.name), tool])).values()];
  const quickScan = normalizeQuickScan(a.quick_scan);

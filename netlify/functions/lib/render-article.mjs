...
function scanHtml(items = []) {
  return items.map((item, index) => `<div class="scan-question"><p>${String(index + 1).padStart(2, '0')} · ${esc(text(item.question))}</p><div class="scan-options">${(item.options || []).map((option) => `<button type="button" data-feedback="${esc(option.feedback || '')}">${esc(text(option.text || option))}</button>`).join('')}</div><p class="scan-feedback" aria-live="polite"></p></div>`).join('');
}

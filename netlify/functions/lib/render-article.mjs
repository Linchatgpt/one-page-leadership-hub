...
function scanHtml(items = []) {
  return items.map((rawItem, index) => { const item = Array.isArray(rawItem) ? { question: rawItem[0], options: rawItem.slice(1) } : typeof rawItem === 'string' ? { question: rawItem } : rawItem || {}; const choices = item.options || item.choices || item.answers || []; return `<div class="scan-question"><p>${String(index + 1).padStart(2, '0')} · ${esc(text(item.question || item.prompt || item.prompt_text || item.title || item.text || item.label))}</p><div class="scan-options">${choices.map((option) => { const value = typeof option === 'string' ? { text: option } : option || {}; return `<button type="button" data-feedback="${esc(value.feedback || '')}">${esc(text(value.text || value.label || value.value))}</button>`; }).join('')}</div><p class="scan-feedback" aria-live="polite"></p></div>`; }).join('');
}
